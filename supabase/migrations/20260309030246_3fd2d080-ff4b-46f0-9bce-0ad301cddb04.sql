...
-- Fix ALL restrictive policies to be PERMISSIVE for conversations
...
CREATE POLICY "Admins can view free access" ON public.free_access_users FOR SELECT TO authenticated USING (has_role(auth.uid(), 'admin'::app_role) OR auth.uid() = user_id);
CREATE POLICY "Admins can grant free access" ON public.free_access_users FOR INSERT TO authenticated WITH CHECK (has_role(auth.uid(), 'admin'::app_role));
CREATE POLICY "Admins can revoke free access" ON public.free_access_users FOR DELETE TO authenticated USING (has_role(auth.uid(), 'admin'::app_role));
